...
- **完整分集覆盖**：生成某一集或一个完整段落分镜时，所有 storyboard item 的 `plannedVideoDurationSeconds` 总和应落在 `project.durationPerEpisode` 的 90%-110%；无特殊剧情理由时优先使用当前模型 `videoDurationCapability.maxSeconds`，具体 item 数量和镜头密度由当前 profile 决定。
...
- 短剧、番剧、电影对镜头密度、对白比例、静默段落和场面调度的要求不同；先读取当前 profile，再决定每个 item 的镜头数量。

...
```bash
# 文生图
cuelight-cli image generate --prompt "赛博朋克城市的猫" --model seedream-4.5 --size 16:9 --output out.png

# 图生图；--image-urls 可传 http(s) URL 或本地文件路径，本地文件由 CLI 自动临时上传
cuelight-cli image generate --prompt "改成夜景霓虹短剧海报质感" --model seedream-4.5 --image-urls ./reference.png --output poster.png

# 文生视频；默认提交异步任务，随后用 task wait 等待

...
- Dialogue Readability Gate（对白读感验收门）：分页后检查每个 ScriptPage 的对白轮替、动作反应和中文术语，避免单页口播或单角色解释性对白。
- Action/Suspense Page Gate（动作悬念页验收门）：检查无对白或少对白页面是否通过“看见 / 听见 / 反应 / 选择 / 危险升级”推进，而不是靠空泛气氛填页。
- Reveal Chain Gate（揭示链验收门）：检查页面信息是否按视觉焦点、声音触发、人物反应和新选择逐步揭示。
- Dialogue Power Turn（对白权力转折）：检查每轮对白是否改变权力、信息、关系、危险或行动，而不是轮流解释设定。
...
- Dialogue readability gate：按根级 `script/pages.yaml` 反查真实 blocks，统计每页 dialogue 数、speaker 数、speaker turns 和 action/dialogue 交错；有对白但单说话人的页必须记录明确例外。
- Action/suspense page gate：无对白页不因 dialogue 数量失败，但必须有清晰视觉焦点、声音或物件触发、人物反应和下一步选择。
- Reveal chain gate：抽读页面是否形成“空间状态 -> 视角/特写/插入 -> 声音或物件变化 -> 人物反应 -> 新危险或剪辑动机”的链条。
- Dialogue power turn：多角色对白页必须逐轮改变权力、信息、关系、危险或行动；只轮流讲设定判 fail。
...
### Dialogue Readability Gate

Dialogue Readability 是正式电影 screenplay 的分页读感验收门。完成 Literary Rewrite、screenplay quality strict 和分页去重后，进入 Commercialization Punch-up、DOCX 导出、分镜或 production-ready 前执行。

执行规则：

1. Root pages as authority：以根级 `script/pages.yaml` 为统计权威；按 `block_refs` 反查真实 `blocks.yaml`，不要只看 scene 内页码或 YAML 总量。
2. Dialogue density：互动场景的 ScriptPage 默认至少包含 2 个 `dialogue` block 和 2 个 speaker turns。动作页可以无对白；有对白但只有 1 个说话人的页，必须是明确的独白、广播、系统语音、审讯压迫、仪式宣告或无对手戏页面，并在 gate 报告中说明。
3. Interleave：对白之间必须有短动作、反应、沉默、道具或声音变化；连续角色口播不能填满一页。
4. Page movement：每页应有一个清晰视觉焦点和一个小推进：新信息、关系压力、危险升级、选择或场尾钩子。
5. Dialogue power：多角色对白不能只是轮流解释设定；每轮台词都应改变权力、信息、关系、危险或行动。若台词删掉后场面不变，继续 Rewrite。
6. Recheck：运行 `check_film_screenplay_quality.py --strict --json`，读取 `dialogueReadability.pages`；若报告 `page_dialogue_underfit`、`single_speaker_dialogue_page`、`dialogue_turn_underfit`、`dialogue_action_interleave_missing` 或 `unlocalized_screenplay_term`，不得标记为 production-ready，除非报告中记录明确例外和 Agent 放行理由。

### Action/Suspense Page Gate

Action/Suspense Page Gate 是少对白、无对白、追踪、潜入、等待、观察、危险逼近等页面的读感验收门。它防止 Dialogue Readability 被误用成“每页必须塞对白”，也防止无对白页变成空泛氛围。

执行规则：

1. No-dialogue allowed：动作页、悬念页、观察页可以没有 `dialogue` block，但不能只是环境描写；页面必须有清晰视觉焦点和可拍推进。
2. Action chain：页面至少形成“看见 / 听见 / 反应 / 选择 / 危险升级”中的三段链条，正式动作悬念页推荐五段齐全。
3. Filmable resistance：动作段必须包含空间阻力、道具状态、声源、视线、身体选择或危险距离中的至少一类具体阻力。
4. Localized labels：需要提示叙事功能时使用中文 `视角：`、`特写：`、`跟拍：`、`声音：`、`画外：`、`旁白：`、`插入：`；这些是叙事功能，不是英文镜头标签翻译腔。
5. Fail patterns：只有气氛、只有人物“紧张”、只有长动作说明、只有抽象危险感，或页面末尾没有新信息、选择、危险升级、剪辑动机，判 fail。

### Reveal Chain Gate

Reveal Chain Gate 检查页面如何揭示信息。它不是要求每页写镜头表，而是要求观众获得信息的路径可拍、可听、可剪。

执行规则：

1. From space to detail：揭示先有空间状态或人物目标，再进入 `视角：`、`特写：`、`插入：` 等具体信息点。
2. Trigger and reaction：揭示点要触发声音、物件变化、视线变化或身体反应；人物不能只“意识到”，要做出可见选择。
3. Cut motivation：页面末尾应形成下一拍剪辑动机，例如新危险、误导被拆穿、关系位置改变、行动目标改变或场尾钩子。
4. Restraint：不要把 `视角：`、`特写：`、`插入：` 当成装饰；只有当它们改变信息、风险、权力或剪辑点时使用。
...
- 不写单角色解释性对白页；不把每页固定成“几段动作 + 一句台词”；不让多角色在场时只有主角持续解释。
- 对白不只讲世界观，必须改变关系、局势、行动或观众信息；动作段不能只是承接对白，要有可拍的阻力、反应、空间调度或声音变化。
- 无对白页必须有“看见 / 听见 / 反应 / 选择 / 危险升级”链条，而不是只有气氛或人物感受。
- 动作段应包含空间阻力、道具状态、声源、视线、身体选择或危险距离；缺少这些可拍信息时继续 Rewrite。
- 页面应有揭示链：视觉焦点或声音触发使人物获得新信息，并迫使场面进入下一步。
...
- 分页后是否出现单角色解释页、连续口播页、固定“动作 + 一句台词”页，或多角色在场但只有一个人持续讲设定。
...
- 无对白页可以成立，但必须用视觉焦点、声音触发、人物反应、道具变化或危险升级推进。
- 功能提示用中文叙事标签：`视角：`、`特写：`、`跟拍：`、`声音：`、`画外：`、`旁白：`、`插入：`；这些标签只在能改变信息、风险、权力或剪辑点时使用。
...
中文剧本正文默认使用本地化功能词：`视角：`、`特写：`、`跟拍：`、`声音：`、`画外：`、`旁白：`、`插入：`。这些标签表示叙事功能，不是英文镜头标签翻译腔；只有当它们改变信息、风险、权力或剪辑点时使用。英文 screenplay 功能词只允许出现在 schema、工具名、workflow 名或必要技术说明中，不进入 `action.text` / `dialogue.text` 等中文成片剧本正文。
...
应急灯一盏接一盏熄灭。林乔贴着档案柜侧面移动，左手压住胸前的门禁卡，右手握着还没拧亮的手电。

视角：门禁屏幕

屏幕上原本静止的红点忽然向资料库移动，距离从三十米跳到二十七米。

声音：电梯井里传来很轻的金属碰撞。

林乔停住。她没有开手电，只把门禁卡塞进档案柜缝里，腾出手去摸地上的碎玻璃。

特写：碎玻璃映出走廊尽头的一线白光。

白光扫过门缝。林乔屏住呼吸，把玻璃片慢慢推向另一侧。白光跟着玻璃反射偏开，照到空置的阅览桌。

声音：门禁锁轻响一声。

林乔抬眼。红点停在门外。

她松开玻璃，转身钻进半开的地档抽屉。抽屉滑轨发出一声短促摩擦。
...
窗外的雨把玻璃打成灰色。周衡站在长桌一端，手套还没摘；许瓷把一只封存袋放到桌面中央，封条完整。

                    周衡
          你要的不是文件，是我签字。

许瓷没有回答。她把录音笔倒扣在封存袋旁边，红灯没有亮。

                    许瓷
          我只要你别把今晚说成例行程序。

周衡看了一眼门口。值班员背对他们整理钥匙，钥匙串碰到铁柜，响得很短。
...
许瓷把封存袋推过去半寸，没有越过桌缝。
                    ...
                    许瓷
          那你签错地方。保我，是你的风险；不签，是你的证词。

周衡摘下一只手套。手套里露出的指尖沾着印泥。
...
天桥下的公交站只剩一盏灯。沈砚站在桥栏边，雨水顺着伞骨滴到鞋尖。他看着对面广告牌上的维修布，布角被风掀起又落下。

跟拍：沈砚沿天桥向西走。

他经过第三根桥柱时，脚步慢下来。桥柱底部的泥水被踩出两种鞋印，一种向站台去，一种停在柱后。

插入：旧公交卡

公交卡卡在排水缝里，卡面磨掉一角，背面贴着半张药房小票。

声音：桥下广播响起，末班车改道通知重复到第二遍。

沈砚抬头。广告牌维修布被风掀开，露出里面一块新刷的白漆；白漆边缘还没干，被雨冲出细线。

特写：药房小票上的时间。

小票时间比广播里的末班车早十二分钟。

沈砚收起伞，把公交卡装进口袋，转身走向桥下站台。远处一辆没有开灯的车缓慢靠边。
```
...
- Dialogue Readability Gate 已记录每页 dialogue blocks、speaker count、speaker turns、action/dialogue 交错和单说话人例外；正式中文正文不含 `SOUND`、`INSERT`、`CLOSE ON`、`ANGLE ON`、`MOVING WITH` 等未本地化功能提示。
- Action/Suspense Page Gate 和 Reveal Chain Gate 已记录无对白页/动作悬念页的视觉焦点、声音触发、人物反应、危险升级或剪辑动机；无对白页不得只靠氛围填充。
- 多角色对白页通过 Dialogue Power Turn 检查：每轮台词改变权力、信息、关系、危险或行动。
